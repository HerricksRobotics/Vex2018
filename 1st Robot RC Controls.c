#pragma config(Motor,  port1,           leftDrive,     tmotorVex393_HBridge, openLoop)
#pragma config(Motor,  port2,           rightDrive,    tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port3,           leftTopBaseArm, tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port4,           leftBottomBaseArm, tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port5,           rightTopBaseArm, tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port6,           rightBottomBaseArm, tmotorVex393_MC29, openLoop, reversed)
//*!!Code automatically generated by 'ROBOTC' configuration wizard               !!*//

task main()
{
	int leftSpeed;
	int rightSpeed;
	int armBaseSpeed;

	/*
	int armLiftSpeed; 				not part of the robot yet
	*/

	while(true)
	{
		//To control the left side using channel 3
		if (vexRT[Ch3] > -35 && vexRT[Ch3] < 35)
		{
			leftSpeed = 0;
		}
		else
		{
			leftSpeed = vexRT[Ch3];
		}


		//To control the right side using channel 2
		if (vexRT[Ch2] > -35 && vexRT[Ch2] < 35)
		{
			rightSpeed = 0;
		}
		else
		{
			rightSpeed = vexRT[Ch2];
		}

		motor[leftWheel] = leftSpeed;
		motor[rightWheel] = rightSpeed;



		//lift up using Button 5U or 6D
		if (vexRT[Btn5U] == 1 || vexRT[Btn6D] == 1)				//moving arm up
		{
			armBaseSpeed = 127;
		}
		else
		{
			armBaseSpeed = 0;
		}

		motor[armLeftFront] = armBaseSpeed;
		motor[armLeftBack] = armBaseSpeed;
		motor[armRightFront] = armBaseSpeed;
		motor[armRightBack] = armBaseSpeed;


		//lift up using Button 5D or 6U
		if (vexRT[Btn5D] == 1 || vexRT[Btn6U] == 1)				//moving arm down
		{
			armSpeed = -127;
		}
		else
		{
			armSpeed = 0;
		}

		motor[armLeftFront] = armBaseSpeed;
		motor[armLeftBack] = armBaseSpeed;
		motor[armRightFront] = armBaseSpeed;
		motor[armRightBack] = armBaseSpeed;

	}


}
